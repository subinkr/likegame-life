-- LikeGame.life 데이터베이스 초기화 및 재생성

-- 1. 기존 테이블들 삭제 (순서 중요: 외래 키 제약 조건 때문에)
...
-- 5. 기존 인덱스들 삭제
DROP INDEX IF EXISTS idx_strength_records_user_id CASCADE;
DROP INDEX IF EXISTS idx_strength_records_created_at CASCADE;
DROP INDEX IF EXISTS idx_stats_user_id CASCADE;
DROP INDEX IF EXISTS idx_stats_month CASCADE;
DROP INDEX IF EXISTS idx_skills_user_id CASCADE;
DROP INDEX IF EXISTS idx_user_badges_user_id CASCADE;
DROP INDEX IF EXISTS idx_user_titles_user_id CASCADE;
DROP INDEX IF EXISTS idx_quests_user_id CASCADE;
DROP INDEX IF EXISTS idx_party_members_party_id CASCADE;
DROP INDEX IF EXISTS idx_wisdom_notes_user_id CASCADE;
DROP INDEX IF EXISTS idx_books_user_id CASCADE;
DROP INDEX IF EXISTS idx_chat_messages_room_id CASCADE;
DROP INDEX IF EXISTS idx_chat_messages_created_at CASCADE;

-- 6. 새로운 테이블들 생성

-- 6-1. 사용자 테이블 (Supabase Auth와 연동)
CREATE TABLE users (
...
-- 6-17. 시스템 메시지 생성 함수 (채팅 관련 테이블 생성 후에 정의)
...
-- 6-18. 시스템 메시지 트리거 생성 (테이블 생성 직후에 생성)
...
-- 9. 기본 RLS 정책 (사용자는 자신의 데이터만 접근 가능)
...
-- 10. 함수 생성 (사용자 생성 시 자동으로 users 테이블에 추가)
...
-- 12. 기존 사용자들을 public.users 테이블에 추가하는 함수
